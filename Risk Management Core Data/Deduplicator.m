...
+ (void)deDuplicateEntityWithName:(NSString*)entityName
    ...
    [importContext performBlock:^{
        ...
        NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] initWithEntityName:entityName];
        NSArray *sortDescriptors = [NSArray arrayWithObjects:[NSSortDescriptor sortDescriptorWithKey:uniqueAttributeName
                                                                                           ascending:YES],nil];
        ...
        for (NSManagedObject *object in duplicateObjects) {
            if (lastObject) {
                if ([[object valueForKey:uniqueAttributeName]
                    ...
                    NSDate *date1 = [object valueForKey:@"updated_at"];
                    NSDate *date2 = [lastObject valueForKey:@"updated_at"];
