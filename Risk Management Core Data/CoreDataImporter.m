...
- (NSManagedObject*)copyUniqueObject:(NSManagedObject*)object
    ...
    if (uniqueAttributeValue.length  > 0) {
        ...
        // Update updated_at date as appropriate
        if ([[[copiedObject entity] attributesByName] objectForKey:@"updated_at"]) {
            [copiedObject setValue:[NSDate date] forKey:@"updated_at"];
        }
        return copiedObject;
    }
    return nil;
    ...
    for (NSString *relationshipName in relationships) {
        ...
        if ([sourceObject valueForKey:relationshipName]) {
            ...
            if (relationship.isToMany && relationship.isOrdered) {
                ...
                NSManagedObject *relatedCopiedObject = [self copyUniqueObject:relatedSourceObject
